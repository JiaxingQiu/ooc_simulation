...
for(rn in c("lm","lr")){
  ...
  agg_df <- agg_df_ls[[rn]]
  agg_df <- agg_df %>% filter(ar1_phi == 0.3, n_obs_per_cluster == 25) %>% as.data.frame()
  ...
  for(st in c("t","loopred","loodev") ){
    ...
    levels(plot_df$sigma_rdm_fix_ratio) <- c("low", "median", "high")
    ...
    plot_ls[[st]] <- ggplot(data = plot_df, aes(x = n_ttl_betas, y = model_m, color = model_type)) + 
      ...
      scale_x_continuous(limits = c(2, 8), 
                         breaks = c(3,5,7)) +
      # coord_trans(y = "sqrt") +
      facet_wrap(~ sigma_rdm_fix_ratio, ncol=3, nrow=1, scales="free_x") + 
      labs(x = NULL, 
           y = ylab,
           color = "Model") + 
      theme_bw()+
...
library(cowplot)
p <- ggdraw(p) +
  draw_label("C. Strength of random effects", x = 0.01, y = 0.99, hjust = 0, vjust = 1, 
