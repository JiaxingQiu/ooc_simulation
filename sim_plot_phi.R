...
for(rn in c("lm","lr")){
  ...
  agg_df <- agg_df_ls[[rn]]
  agg_df <- agg_df %>% filter(sigma_rdm_fix_ratio == 1, n_obs_per_cluster == 25) %>% as.data.frame()
  ...
  for(st in c("t","loopred","loodev") ){
    ...
    # change levels here
    plot_df$ar1_phi <- factor(plot_df$ar1_phi, levels=sort(unique(plot_df$ar1_phi)) )
    levels(plot_df$ar1_phi) <- c("low", "median", "high")
    ...
    plot_ls[[st]] <- ggplot(data = plot_df, aes(x = n_ttl_betas, y = model_m, color = model_type)) + 
      ...
      # coord_trans(y = "sqrt") +
      facet_wrap(~ ar1_phi, ncol=3, nrow=1, scales="free_x") + 
      labs(x = NULL, 
           y = ylab,
           color = "Model") + 
      theme_bw()+
...
library(cowplot)
p <- ggdraw(p) +
  draw_label("B. Within-cluster autocorrelation", x = 0.01, y = 0.99, hjust = 0, vjust = 1, 
